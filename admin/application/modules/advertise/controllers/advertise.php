  ...
  function index($sort_by='',$perpage=0,$offset=0)
		...
		else
		{
            $uid=$this->session->userdata('userid');
					...
					$data['order']=$sort_by;
					$numrosw=$this->db->query("select * from sch_advertise")->num_rows();
					
					$config['base_url'] = base_url()."advertise/index/$sort_by/$perpage";                
				   ...
				   $data['results']=$this->db->query("select * from sch_advertise $order limit $offset,$limit")->result_array();
                                   $this->load->view('advertise_home',$data);
   ...
   { 
	   ...
	   if(!$this->session->userdata('userid'))
		...
		else
		{
			...
			else
			{
 			...
 			$data['order']='';
            $numrosw=$this->db->query("select * from sch_advertise where (image_title like '%".$search_text."%' OR area_name like '%".$search_text."%')")->num_rows();
            
			$config['base_url'] = base_url()."advertise/index/$search_text";                
			$config['total_rows'] = $numrosw;
                        $config['per_page'] = $perpage;   
			$config['page_query_string']='FALSE';
		   ...
		   $data['results']=$this->db->query("select * from sch_advertise where (image_title like '%".$search_text."%' OR area_name like '%".$search_text."%') order by id desc limit $offset,$limit")->result_array();
	       $this->load->view('advertise_home',$data);
    ...
    public function add()
    {    
        $data['citys']=$this->Advertise_model->allstates();
        ...
        if(!$this->session->userdata('userid'))
		{
			...
			$uid=$this->session->userdata('userid');			
					if(isset($_POST['advertiseForm'])) {                                           						
						
					      $this->Advertise_model->add_advertise();	
					      $this->session->set_flashdata('success', '<div style="line-height: 0;height: 23px; padding-left: 17px;" class="alert alert-success"><strong>Success! </strong>Advertise has been added successfully..! </div>');
					     redirect('advertise');
					}
					else
					{
	 ...
	 if(!$this->session->userdata('userid'))
		...
		else
		{
				...
				{
	                 $data['results']=$this->Advertise_model->oneRecordById($banner_id,"sch_advertise");                         
                         $this->load->view('advertise_edit',$data);
				}
		}
	 }
		...
		if(!$this->session->userdata('userid'))
			{
				redirect('user/login');
                        ...
                        if(isset($_POST['advertiseForm'])) 
                        {                       
				 $this->Advertise_model->update_advertise();	
