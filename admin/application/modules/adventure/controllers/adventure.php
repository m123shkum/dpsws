...
class Adventure extends MX_Controller {
 public $droot = "";
  ...
  function index($sort_by='',$perpage=0,$offset=0)
		...
		else
		{
            $uid=$this->session->userdata('userid');
					...
					$data['order']=$sort_by;
					$numrosw=$this->db->query("select * from sch_adventure")->num_rows();
					
					$config['base_url'] = base_url()."adventure/index/$sort_by/$perpage";                
				   ...
				   $data['results']=$this->db->query("select * from sch_adventure $order limit $offset,$limit")->result_array();
                                   $this->load->view('adventure_home',$data);
   ...
   { 
	   ...
	   if(!$this->session->userdata('userid'))
		...
		else
		{
			...
			else
			{
 			...
 			$data['order']='';
            $numrosw=$this->db->query("select * from sch_adventure where (title like '%".$search_text."%' OR adventure_area like '%".$search_text."%')")->num_rows();
            
			$config['base_url'] = base_url()."adventure/index/$search_text";                
			$config['total_rows'] = $numrosw;
                        $config['per_page'] = $perpage;   
			$config['page_query_string']='FALSE';
		   ...
		   $data['results']=$this->db->query("select * from sch_adventure where (title like '%".$search_text."%' OR adventure_area like '%".$search_text."%') order by id desc limit $offset,$limit")->result_array();
	       $this->load->view('adventure_home',$data);
    ...
    {    
        ...
        if(!$this->session->userdata('userid'))
		{
			...
			$uid=$this->session->userdata('userid');			
					if(isset($_POST['adventureForm'])) {                                           						
						
					      $this->Adventure_model->add_adventure();	
					...
					{
					 $this->load->view('adventure_add',$data);
					}
	 ...
	 if(!$this->session->userdata('userid'))
		...
		else
		{
				...
				{
	                 $data['results']=$this->Adventure_model->oneRecordById($page_id,"sch_adventure");                         
                         $this->load->view('adventure_edit',$data);
				}
		}
	 }
		...
		if(!$this->session->userdata('userid'))
			{
				redirect('user/login');
                        ...
                        if(isset($_POST['adventureForm'])) 
                        {                       
				 $this->Adventure_model->update_adventure();	
    ...
    function delete() {
		  if(isset($_POST['delid']))
		  {
                      ...
                      $this->db->query("DELETE from sch_adventure where id=".$_POST['delid']."");
