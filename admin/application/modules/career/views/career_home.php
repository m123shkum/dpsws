...
<div id="page-wrapper">
  <div class="row">
    <div class="col-lg-12">
      <h1><small>Manage Career</small></h1>
       <ol class="breadcrumb">
        <li><a href="<?php echo base_url();?>user/home"><i class="icon-dashboard"></i> Dashboard</a></li>        
        <li class="active"><i class="icon-file-alt"></i>Career</li>
      </ol>
      <div class="row-fluid">
        <div class="span6">              
            <div class="pull-right" style="max-width: 570px;">
            <label class="inner-label">Search by Name Or Email Or Applied For:</label>
  ...
  <div class="panel panel-default"> 
    ...
    <table class="table">
      ...
      <tr <?php echo $class;?>>
        ...
        <td><?php echo $data['appliedfor'];?></td> 
        <td>
          <?php if($data['resume']!=''){?>
          <a href="<?php echo SiteUrl;?>public/career_resume/<?=$data['resume']?>">Download Resume</a>
          <?php }else{?>
          No Resume
          <?php }?>
      </td>
...
var sort_by=document.getElementById("sort_by").value;
  ...
  window.location='<?=base_url()?>career/index/'+sort_by+'/'+val+'/0'; 
}
function pageSort(sort_by) { 
	  window.location='<?=base_url()?>career/index/'+sort_by+'/0/0';
}
function serachAdmin()
{
	...
	window.location='<?=base_url()?>career/search/'+search_text+'/0';
	return false;
}
function getVal(id)
{
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form action="<?=base_url()?>career/delete" method="post">           
           <input type="hidden" name="delid"  id="delid" value="">
           <input type="hidden" name="curl"  id="curl" value="<?php echo base_url();?>career">
