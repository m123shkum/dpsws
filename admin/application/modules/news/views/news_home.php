...
<div id="page-wrapper">
    ...
    <div class="col-lg-12">
      <h1><small>Manage News</small></h1>
       <ol class="breadcrumb">
        ...
        <li class="active"><i class="icon-file-alt"></i>News</li>
      </ol>
      <div class="row-fluid">
        <div class="span6">              
          <div class="pull-right">
            <label class="inner-label">Search by News title:</label>
          ...
          <a style="text-decoration:none;" href="<?=base_url().'news/add/'?>"><span class="btn add-btn"></span></a>
          ...
          <div class="sort-row">
            ...
            <select onchange="pageSort(this.value)" style="width:140px;" class="form-control per" name="sort_by" id="sort_by">
              ...
              <option <?php if($order=='city-asc'){ echo "selected='selected'";}?> value="city-asc">City asc</option>
              <option <?php if($order=='city-desc'){ echo "selected='selected'";}?> value="city-desc">City desc</option>
            </select>
            </div>
          </div>
  ...
  <div class="panel panel-default"> 
    ...
    <table class="table">
      ...
      <tr <?php echo $class;?>>
        <td><?php echo $j;?></td>
        <td><?php echo stripslashes(ucwords($data['news_title']));?></td>    
        <td><?php echo ucwords($data['news_area']);?></td> 
        <td> 
         ...
         <?php } ?>
          <a rel="tooltip" alt="Update" href="<?=base_url()?>news/edit/<?=$data['id']?>"  class="btn btn-xs btn-info" data-original-title="Update">
...
<!--===================status changes form =====-->
    <form action="<?=base_url()?>news/changeStatus" method="post" id="cngststus">
...
var sort_by=document.getElementById("sort_by").value;
  ...
  window.location='<?=base_url()?>news/index/'+sort_by+'/'+val+'/0'; 
}
function pageSort(sort_by) { 
	  window.location='<?=base_url()?>news/index/'+sort_by+'/0/0';
}
function serachAdmin()
{
	...
	window.location='<?=base_url()?>news/searchevent/'+search_text;
	return false;
}
function getVal(id)
{
