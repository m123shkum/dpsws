	   ...
	   if(!$this->session->userdata('userid'))
		...
		else
		{
			...
			else
			{
 			...
 			$data['order']='';
                        //if($_REQUEST[''])
            $numrosw=$this->db->query("select * from sch_school where (school_name like '%".$searchtext."%' or area_name like '%".$searchtext."%' or location like '%".$searchtext."%')$paidby")->num_rows();
            
			$config['base_url'] = base_url()."school/search/$search_text";                
			$config['total_rows'] = $numrosw;
                        $config['per_page'] = $perpage;   
			$config['page_query_string']='FALSE';
	       ...
	       $limit=$perpage;
	       $data['links']=$this->pagination->create_links();
		   $search_text=str_replace('-',' ',$searchtext);
		   $search_text=str_replace('~','@',$search_text);
		   $data['search_text']=$search_text;
                   $data['paidby']=$arr[1];
                   $data['startlimit']=$offset;
                  
		   $data['results']=$this->db->query("select * from sch_school where (school_name like '%".$searchtext."%' or area_name like '%".$searchtext."%' or location like '%".$searchtext."%')$paidby order by id desc limit $offset,$limit")->result_array();
	       $this->load->view('school_home',$data);
       ...
       if(isset($_POST['importForm']))
       {
        ...
        {   
           ...
           // $userres=$this->db->query("SELECT user_id from sch_school where id='".$data[0]."'")->row_array();
            $num=1;
			//
			$num1=$this->db->query("SELECT * from sch_zone where location='".$data[8]."'")->num_rows();
                     
                      $location=ucfirst($data[8]);
                      
                    if($num1==0)
                    {                        
                     $this->db->query("INSERT into sch_zone(zone_locationid,location) values(".$data[7].",'".$location."')");
                    }
                    
                   //echo "select * from sch_zone where zone_id=".$data[6]."";
                
                  $res=$this->db->query("select * from sch_zone where zone_id=".$data[6]."")->row_array();
                  $state= $res['name'];
                  $res=$this->db->query("select * from sch_zone where zone_id=".$data[7]."")->row_array();
                  $area= $res['cityname'];
			//
			
            $address=mysql_real_escape_string(ucwords(strtolower($data[2])));
           ...
           {            
                ...
                $this->db->query("UPDATE sch_school set seo_url='".$seo_url."',school_name='".$school_name."',address ='".$address."',board ='".$data[3]."',category ='".$data[4]."',sub_category ='".$data[5]."',state_id ='".$data[6]."',state_name='".$state."',area_id ='".$data[7]."',area_name='".$area."',location ='".$data[8]."',latitude ='".$data[9]."',langitude ='".$data[10]."' where id=".$data[0]."");    
