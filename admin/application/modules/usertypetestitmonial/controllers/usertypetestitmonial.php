...
class Usertypetestitmonial extends MX_Controller {
 public $droot = "";
  function __construct()
       {
           parent::__construct();
		   $this->load->model('user_testimonial_model');
  ...
  function index($perpage=0,$offset=0)
		...
		else
		{
            $uid=$this->session->userdata('userid');
					...
					$numrosw=$this->db->query("select * from sch_usertype_testimonial")->num_rows();
					
					$config['base_url'] = base_url()."usertypetestitmonial/index/$perpage";                
				   ...
				   $data['results']=$this->db->query("select * from sch_usertype_testimonial $order limit $offset,$limit")->result_array();
                                   $this->load->view('usertestimonial_home',$data);
		}
        }
   ...
   { 
    ...
    {    
        ...
        if(!$this->session->userdata('userid'))
		{
			...
			$uid=$this->session->userdata('userid');			
					if(isset($_POST['testimonialForm'])) {                                           						
					      ...
					      $this->user_testimonial_model->add_testimonial();	
					     ...
					     redirect('usertypetestitmonial');
					}
					else
					{
					 $this->load->view('usertestimonial_add',$data);
					}
	 ...
	 if(!$this->session->userdata('userid'))
		...
		else
		{
				...
				{
	                 $data['results']=$this->user_testimonial_model->oneRecordById($page_id,"sch_usertype_testimonial");                         
                     $this->load->view('usertestimonial_edit',$data);
				}
		}
	 }
		...
		if(!$this->session->userdata('userid'))
			{
		      redirect('user/login');
				 ...
				 $this->user_testimonial_model->update_testimonial();	
    ...
    function delete() {
		  if(isset($_POST['delid']))
		  {
                      ...
                      $this->db->query("DELETE from  sch_usertype_testimonial where id=".$_POST['delid']."");
